...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	status = XSpi_Transfer(&SpiInstance, buffer, read_buffer, 2);
	if(status != XST_SUCCESS){
		xil_printf("Transfer Error in wr_single\r\n");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	status = XSpi_Transfer(&SpiInstance, buffer, NULL, nbytes+1);
	if(status != XST_SUCCESS){
		xil_printf("Transfer Error in wr_multi\r\n");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE read_val[2] = {0, 0};
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	status = XSpi_Transfer(&SpiInstance, buffer, read_val, 2);
	if(status != XST_SUCCESS){
		xil_printf("Transfer Error in read_single%d\r\n", status);
		XSpi_SetSlaveSelect(&SpiInstance, 0);
		return 0;
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int status, i;
	BYTE buffer[nbytes+1];
	BYTE read_buffer[nbytes+1];
	for(i=0;i<nbytes+1;i++){
		buffer[i]=0;
	}
	buffer[0]=reg;
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	status = XSpi_Transfer(&SpiInstance, buffer, read_buffer, nbytes+1);
		if(status != XST_SUCCESS){
			xil_printf("Transfer Error in read_multi\r\n");
		}
	for(i=0;i<nbytes;i++){
