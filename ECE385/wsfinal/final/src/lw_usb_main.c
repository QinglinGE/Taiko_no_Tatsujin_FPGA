...
#include <xgpio.h>
#include "hex_array_chop.h"

extern HID_DEVICE hid_device;
extern uint16_t byteArray[];

static XGpio Gpio_hex;
volatile uint16_t* audio_gpio_data = (volatile uint16_t*)XPAR_GPIO_AUDIO_BASEADDR;
volatile uint8_t* almost_empty = (volatile uint8_t*)XPAR_GPIO_AL_EMPTY_BASEADDR;
volatile uint8_t* almost_full = (volatile uint8_t*)XPAR_GPIO_AL_FULL_BASEADDR;
//volatile uint8_t* empty = (volatile uint8_t*)XPAR_GPIO_EMPTY_BASEADDR;
volatile uint8_t* ready_sig = (volatile uint8_t*)XPAR_GPIO_READY_BASEADDR;
volatile uint16_t* data_count = (volatile uint16_t*)XPAR_GPIO_DATA_COUNT_BASEADDR;
...
int main() {
    ...
    XGpio_Initialize(&Gpio_hex, XPAR_GPIO_USB_KEYCODE_DEVICE_ID);
   	...
   	size_t array_size = sizeof(byteArray);
   	BYTE rcode;
	...
	USB_init();
	while (1) {
//		xil_printf("almost empty: %d\n", *almost_empty);
//		if(*almost_empty && *ready_sig){
//			while(!*almost_full){
//
//				if(count >= array_size) count = 0;
//				*audio_gpio_data = byteArray[count];
////				sleep(1);
//				xil_printf("data count: %d audio_data: %d\n", *data_count, *audio_gpio_data);
//				count += 1;
//			}
//			xil_printf("outside while count: %d data c: %d\n", count, *data_count);
//		}
